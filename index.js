...
- If you decide to add ANY breakdown/tally by a property value as part of your own answer (e.g. "10 are ICP, 109 are not") — even if the user didn't ask for a breakdown — treat it as a Group-By query: use one count_objects call per distinct value (see the Group-By / Aggregate Queries playbook below). NEVER manually tally a property's values from a search_objects results array, especially one already truncated by the 100-result cap — it WILL undercount.
...
- For result sets with more than 20 records, show a grouped summary (e.g. by stage, country, or source) with counts — UNLESS the user's own wording explicitly asks for "a list" / "list all" / "show me all" / similar, in which case show the actual records as a real list/table instead of substituting a count breakdown for what they asked for. If the total exceeds the 100-per-page cap, paginate with the after cursor to retrieve EVERY matching record (up to a sane limit, e.g. 300) before presenting the list — do not stop at the first page just because it hit the cap. Only fall back to a summary if the true total exceeds that sane limit, and say so explicitly if you do.
