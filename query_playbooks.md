...
- "Meetings booked" / "first meeting happened" in [period] → PREFER querying HubSpot's Meetings engagement object directly, since it captures every booked meeting whether or not a deal exists yet: call get_object_properties ONCE with object_type: "meetings", include_internal: true to confirm the timestamp property (commonly hs_timestamp), then sanity-check with ONE count_objects call (object_type: "meetings", no filter) to confirm meetings are actually logged on this portal — if that returns 0 or a suspiciously tiny number, meetings likely aren't tracked this way here. If meetings ARE logged, filter by that timestamp property GTE/LTE the period using search_objects/count_objects with object_type: "meetings" — do NOT combine this with the deal-stage proxy below in the same answer.
- Only if the Meetings object is confirmed empty/unused, fall back to the deal-stage proxy: hs_date_entered_appointmentscheduled on Deal (when the deal entered Objective Win) — and say explicitly in **Notes:** that this only counts meetings that already have an associated deal, so meetings booked before deal creation are undercounted.
