...
The definitions above tell you CURRENT status only, not WHEN a deal entered that stage — dealstage has no built-in transition timestamp. Before combining a milestone with a date range, call get_object_properties ONCE with object_type: "deals", include_internal: true, query: "date_entered" — HubSpot auto-generates hs_date_entered_<stageId> properties per pipeline stage on most portals; reuse this one result for the rest of the question, don't re-check it.
- "Meetings booked" / "first meeting happened" in [period] → the relevant timestamp is hs_date_entered_appointmentscheduled (when the deal entered Objective Win, where the first meeting is canonically scheduled) — use THIS property regardless of the deal's current stage.
- For other milestones ("reached Commercial Win", "hit Closed Won"), use that specific stage's own hs_date_entered_<stageId>.
- If the relevant property is genuinely unavailable, do NOT silently substitute createdate (that's when the deal record was created, not when it reached the stage) — say so explicitly in **Notes:** and offer the closest honest alternative as an explicit choice.
